...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-6 sm:space-y-8" x-data="fanCenterData({{ $featuredMatch ? $featuredMatch->id : 'null' }})">

        <!-- Category Selector Bar -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 pb-4 border-b border-court-border">
            <div>
                <h1 class="text-xl sm:text-2xl font-headline font-bold uppercase tracking-wider text-text-primary flex items-center gap-2">
                    <span class="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-stadium-emerald animate-pulse flex-shrink-0"></span>
                    <span>Pusat Pertandingan & Fan Center</span>
                </h1>
                <p class="text-[10px] sm:text-xs font-mono text-text-muted mt-0.5">TELEMETRY SCOREBOARD & LIVE MATCH TRACKER</p>
            </div>
        ...
        @if ($featuredMatch)
            <div class="relative rounded-xl sm:rounded-2xl overflow-hidden bg-court-surface border border-court-border shadow-2xl p-4 sm:p-6 lg:p-8">
                <!-- Background Glow Effect -->
                <div class="absolute -top-24 -left-24 w-72 sm:w-96 h-72 sm:h-96 bg-stadium-emerald/10 rounded-full blur-3xl pointer-events-none"></div>
                <div class="absolute -bottom-24 -right-24 w-72 sm:w-96 h-72 sm:h-96 bg-telemetry-cyan/10 rounded-full blur-3xl pointer-events-none"></div>

                <div class="relative z-10">
                    <!-- Status Top Bar -->
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 pb-4 sm:pb-6 mb-4 sm:mb-6 border-b border-court-border/60">
                        <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                            <span class="px-2.5 sm:px-3 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs font-mono font-bold tracking-wider border {{ $featuredMatch->status_badge['color'] }}"
                            ...
                            <span class="text-[11px] sm:text-xs font-mono text-text-muted truncate">
                                {{ $featuredMatch->venue }} &bull; {{ $featuredMatch->match_date->format('d M Y, H:i') }} WIB
                            </span>
                        </div>
                    ...
                    <div class="grid grid-cols-3 md:grid-cols-11 items-center gap-2 sm:gap-4 md:gap-6 text-center">
                        ...
                        <div class="col-span-1 md:col-span-4 flex flex-col md:flex-row items-center justify-center md:justify-end gap-2 sm:gap-4 min-w-0">
                            <div class="text-center md:text-right order-2 md:order-1 min-w-0">
                                <h2 class="text-xs sm:text-base md:text-2xl font-headline font-bold text-text-primary leading-tight line-clamp-2" title="{{ $featuredMatch->homeTeam->name }}">
                                    {{ $featuredMatch->homeTeam->name }}
                                </h2>
                                <p class="text-[9px] sm:text-xs font-mono text-text-muted mt-0.5">KANDANG (HOME)</p>
                            </div>
                            <div class="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-xl bg-court-surface-elevated border border-court-border flex items-center justify-center font-headline font-black text-lg sm:text-2xl text-stadium-emerald shadow-inner order-1 md:order-2 overflow-hidden p-1 sm:p-1.5 flex-shrink-0">
                                @if ($featuredMatch->homeTeam->logo)
                                    <img src="{{ $featuredMatch->homeTeam->logo_url }}" alt="{{ $featuredMatch->homeTeam->name }}" class="w-full h-full object-contain">
                                @else
                        ...
                        <div class="col-span-1 md:col-span-3 flex flex-col items-center justify-center py-2 sm:py-4 bg-court-navy/80 rounded-xl border border-court-border/60 px-1 sm:px-3">
                            <div class="flex items-center justify-center gap-1 sm:gap-3 md:gap-4 font-headline text-3xl sm:text-4xl md:text-6xl font-black tracking-tight">
                                <span class="text-stadium-emerald" x-text="featured.home_score">{{ $featuredMatch->home_score }}</span>
                        ...
                        <div class="col-span-1 md:col-span-4 flex flex-col md:flex-row items-center justify-center md:justify-start gap-2 sm:gap-4 min-w-0">
                            <div class="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-xl bg-court-surface-elevated border border-court-border flex items-center justify-center font-headline font-black text-lg sm:text-2xl text-telemetry-cyan shadow-inner overflow-hidden p-1 sm:p-1.5 flex-shrink-0">
                                @if ($featuredMatch->awayTeam->logo)
                                    <img src="{{ $featuredMatch->awayTeam->logo_url }}" alt="{{ $featuredMatch->awayTeam->name }}" class="w-full h-full object-contain">
                                @else
                                    {{ $featuredMatch->awayTeam->initials }}
                                @endif
                            </div>
                            <div class="text-center md:text-left min-w-0">
                                <h2 class="text-xs sm:text-base md:text-2xl font-headline font-bold text-text-primary leading-tight line-clamp-2" title="{{ $featuredMatch->awayTeam->name }}">
                    ...
                    <div class="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-court-border/60 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
                        <div class="text-[10px] sm:text-xs font-mono text-text-muted flex items-center gap-1.5 text-center sm:text-left">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
            ...
            <div class="lg:col-span-7 space-y-6">
                <div class="bg-court-surface rounded-xl border border-court-border overflow-hidden" x-data="{ activeTab: 'all' }">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b border-court-border bg-court-surface-elevated/40">
                        ...
                        <div class="flex items-center gap-1 bg-court-navy p-1 rounded border border-court-border text-xs font-mono overflow-x-auto max-w-full scrollbar-none">
                            <button @click="activeTab = 'all'"
                                :class="activeTab === 'all' ? 'bg-court-surface-elevated text-stadium-emerald font-bold' : 'text-text-muted hover:text-text-primary'"
                                class="px-2.5 sm:px-3 py-1 rounded transition-colors whitespace-nowrap">
                                SEMUA ({{ $allMatches->count() }})
                            </button>
                            <button @click="activeTab = 'live'"
                                :class="activeTab === 'live' ? 'bg-court-surface-elevated text-live-pulse font-bold' : 'text-text-muted hover:text-text-primary'"
                                class="px-2.5 sm:px-3 py-1 rounded transition-colors whitespace-nowrap">
                                LIVE ({{ $liveMatches->count() }})
                            </button>
                            <button @click="activeTab = 'upcoming'"
                                :class="activeTab === 'upcoming' ? 'bg-court-surface-elevated text-telemetry-cyan font-bold' : 'text-text-muted hover:text-text-primary'"
                                class="px-2.5 sm:px-3 py-1 rounded transition-colors whitespace-nowrap">
                                TERJADWAL ({{ $upcomingMatches->count() }})
                            </button>
                            <button @click="activeTab = 'finished'"
                                :class="activeTab === 'finished' ? 'bg-court-surface-elevated text-text-primary font-bold' : 'text-text-muted hover:text-text-primary'"
                                class="px-2.5 sm:px-3 py-1 rounded transition-colors whitespace-nowrap">
                    ...
                    <div class="p-3 sm:p-6 space-y-3 sm:space-y-4">
                        @forelse($allMatches as $match)
                            <div x-show="activeTab === 'all' || (activeTab === 'live' && {{ $match->isLive() ? 'true' : 'false' }}) || (activeTab === 'upcoming' && '{{ $match->status }}' === 'scheduled') || (activeTab === 'finished' && '{{ $match->status }}' === 'finished')"
                                ...
                                <div class="flex items-center justify-between text-[10px] sm:text-[11px] font-mono text-text-muted pb-2.5 mb-2.5 border-b border-court-border/40 gap-2">
                                    <span class="truncate">{{ $match->match_date->format('d M Y - H:i') }} WIB &bull; {{ $match->venue }}</span>
                                    <span class="px-2 py-0.5 rounded text-[10px] font-bold border flex-shrink-0 {{ $match->status_badge['color'] }}">
                                ...
                                <div class="grid grid-cols-7 items-center gap-1.5 sm:gap-2">
                                    ...
                                    <div class="col-span-3 flex items-center gap-2 sm:gap-3 min-w-0">
                                        <div class="w-7 h-7 sm:w-8 sm:h-8 rounded bg-court-navy border border-court-border flex items-center justify-center font-headline font-bold text-[10px] sm:text-xs text-stadium-emerald flex-shrink-0 overflow-hidden p-0.5">
                                        ...
                                        <span class="font-headline font-bold text-xs sm:text-sm text-text-primary truncate" title="{{ $match->homeTeam->name }}">{{ $match->homeTeam->name }}</span>
                                    </div>

                                    <!-- Score -->
                                    <div class="col-span-1 text-center font-headline font-black text-sm sm:text-lg">
                                        @if ($match->status === 'scheduled')
                                            <span class="text-text-muted text-[10px] sm:text-xs font-mono font-normal">VS</span>
                                        @else
                                    ...
                                    <!-- Away -->
                                    <div class="col-span-3 flex items-center justify-end gap-2 sm:gap-3 text-right min-w-0">
                                        <span class="font-headline font-bold text-xs sm:text-sm text-text-primary truncate" title="{{ $match->awayTeam->name }}">{{ $match->awayTeam->name }}</span>
                                        <div class="w-7 h-7 sm:w-8 sm:h-8 rounded bg-court-navy border border-court-border flex items-center justify-center font-headline font-bold text-[10px] sm:text-xs text-telemetry-cyan flex-shrink-0 overflow-hidden p-0.5">
                                ...
                                <!-- Actions bar -->
                                <div class="mt-2.5 sm:mt-3 pt-2.5 sm:pt-3 border-t border-court-border/40 flex flex-col sm:flex-row sm:items-center justify-between text-xs font-mono gap-1.5 sm:gap-2">
                                    <span class="text-text-muted text-[10px] sm:text-[11px] truncate">
                                        {{ $match->stage?->name }} {{ $match->group ? '&bull; ' . $match->group->name : '' }}
                                    </span>
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('matches.show', $match->id) }}" class="text-stadium-emerald hover:underline text-[11px] sm:text-xs flex items-center gap-1 font-headline font-medium">
                ...
                @if (count($bracketStages) > 0)
                    <div class="bg-court-surface rounded-xl border border-court-border p-4 sm:p-6 space-y-4">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-court-border gap-2">
                            <h3 class="font-headline font-bold uppercase tracking-wider text-sm flex items-center gap-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            @foreach ($bracketStages as $bStage)
                                <div class="p-3.5 sm:p-4 rounded-lg bg-court-surface-elevated border border-court-border space-y-3">
                                    <div class="text-xs font-headline font-bold text-stadium-emerald uppercase border-b border-court-border/40 pb-2">
                                        {{ $bStage['stage']->name }}
                                    </div>
                                        ...
                                        <div class="p-2.5 rounded bg-court-navy/70 border border-court-border/60 text-xs">
                                            <div class="flex justify-between items-center py-1">
                                                <div class="flex items-center gap-2 truncate min-w-0">
                                                    <div class="w-5 h-5 rounded bg-court-surface border border-court-border flex items-center justify-center font-headline font-bold text-[9px] text-stadium-emerald flex-shrink-0 overflow-hidden">
                                                        @if ($bm->homeTeam->logo)
                                                            <img src="{{ $bm->homeTeam->logo_url }}" alt="{{ $bm->homeTeam->name }}" class="w-full h-full object-contain">
                                                        @else
                                            ...
                                            <div class="flex justify-between items-center py-1 border-t border-court-border/30">
                                                <div class="flex items-center gap-2 truncate min-w-0">
                                                    <div class="w-5 h-5 rounded bg-court-surface border border-court-border flex items-center justify-center font-headline font-bold text-[9px] text-telemetry-cyan flex-shrink-0 overflow-hidden">
                                                        @if ($bm->awayTeam->logo)
                                                            <img src="{{ $bm->awayTeam->logo_url }}" alt="{{ $bm->awayTeam->name }}" class="w-full h-full object-contain">
                                                        @else
            ...
            <div class="lg:col-span-5 space-y-6">
                ...
                <div class="bg-court-surface rounded-xl border border-court-border overflow-hidden"
                    ...
                    <!-- Standings Header -->
                    <div class="px-4 sm:px-6 py-4 border-b border-court-border bg-court-surface-elevated/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            <h3 class="font-headline font-bold uppercase tracking-wider text-sm flex items-center gap-2">
                                <span class="material-symbols-outlined text-stadium-emerald text-base">format_list_numbered</span>
                                Klasemen Turnamen
                            </h3>
                            ...
                            <div class="flex items-center gap-1 bg-court-navy p-1 rounded border border-court-border text-xs font-mono overflow-x-auto max-w-full scrollbar-none">
                                @foreach ($standingsByGroup as $gName => $gData)
                                    <button @click="activeGroup = '{{ $gName }}'"
                                        :class="activeGroup === '{{ $gName }}' ? 'bg-stadium-emerald text-court-navy font-bold' : 'text-text-muted hover:text-text-primary'"
                    ...
                    <div class="sm:hidden px-4 py-1.5 bg-court-navy/50 text-[10px] font-mono text-text-muted flex items-center justify-between border-b border-court-border/30">
                    ...
                    @foreach ($standingsByGroup as $gName => $gData)
                        <div x-show="activeGroup === '{{ $gName }}'" class="overflow-x-auto scrollbar-none">
                            @if (count($standingsByGroup) > 1)
                                <div class="px-4 sm:px-6 py-2 bg-court-surface-elevated/20 border-b border-court-border text-xs font-headline font-bold text-telemetry-cyan uppercase">
                                    {{ $gName }}
                                </div>
                            @endif

                            <table class="w-full text-left text-xs min-w-[480px] sm:min-w-full">
                                <thead class="bg-court-navy/80 font-mono text-[11px] text-text-muted uppercase border-b border-court-border">
                                    <tr>
                                        ...
                                        <th class="py-2.5 px-3 text-right font-bold text-stadium-emerald" title="Poin">PTS</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-court-border/40 font-mono">
                                    @forelse($gData['rows'] as $row)
                                        <tr class="hover:bg-court-surface-elevated/40 transition-colors {{ $row['position'] <= 2 ? 'border-l-2 border-l-stadium-emerald' : '' }}">
                                            <td class="py-3 px-3 text-center font-bold {{ $row['position'] <= 2 ? 'text-stadium-emerald' : 'text-text-muted' }}">
                                                {{ $row['position'] }}
                                            </td>
                                            <td class="py-3 px-3 font-sans font-medium text-text-primary">
                                                <div class="flex items-center gap-2">
                                                    <div class="w-6 h-6 rounded bg-court-navy border border-court-border flex items-center justify-center font-headline font-bold text-[10px] text-stadium-emerald flex-shrink-0 overflow-hidden p-0.5">
                                                        @if ($row['team']->logo)
                                                            <img src="{{ $row['team']->logo_url }}" alt="{{ $row['team']->name }}" class="w-full h-full object-contain">
                                                        @else
                                                            {{ $row['team']->initials }}
                                                        @endif
                                                    </div>
                                                    <span class="truncate max-w-[120px] sm:max-w-none">{{ $row['team']->name }}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td class="py-3 px-2 text-center text-text-muted">{{ $row['goals_against'] }}</td>
                                            <td class="py-3 px-2 text-center font-bold {{ $row['goal_diff'] > 0 ? 'text-stadium-emerald' : ($row['goal_diff'] < 0 ? 'text-card-red' : 'text-text-muted') }}">
                                                {{ $row['goal_diff'] > 0 ? '+' . $row['goal_diff'] : $row['goal_diff'] }}
                                            </td>
                                            <td class="py-3 px-3 text-right font-headline font-black text-sm text-stadium-emerald">
                                                {{ $row['points'] }}
                                            </td>
                                        </tr>
                                    @empty
                ...
                <div class="bg-court-surface rounded-xl border border-court-border overflow-hidden"
                    ...
                    <div class="px-4 sm:px-6 py-4 border-b border-court-border bg-court-surface-elevated/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <div class="flex items-center gap-1 bg-court-navy p-1 rounded border border-court-border text-xs font-mono w-full sm:w-auto justify-center">
                            <button @click="statTab = 'scorers'"
                                :class="statTab === 'scorers' ? 'bg-stadium-emerald text-court-navy font-bold' : 'text-text-muted hover:text-text-primary'"
                                class="flex-1 sm:flex-initial px-2.5 py-1 rounded transition-colors text-center">
                                GOL
                            </button>
                            <button @click="statTab = 'assists'"
                                :class="statTab === 'assists' ? 'bg-telemetry-cyan text-court-navy font-bold' : 'text-text-muted hover:text-text-primary'"
                                class="flex-1 sm:flex-initial px-2.5 py-1 rounded transition-colors text-center">
                                ASSIST
                            </button>
                            <button @click="statTab = 'cards'"
                                :class="statTab === 'cards' ? 'bg-card-yellow text-court-navy font-bold' : 'text-text-muted hover:text-text-primary'"
                                class="flex-1 sm:flex-initial px-2.5 py-1 rounded transition-colors text-center">
                                KARTU
                            </button>
                        </div>
                        ...
                        @forelse($topScorers as $idx => $sc)
                            <div class="py-2.5 flex items-center justify-between gap-2">
                                <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                                    <span class="w-5 sm:w-6 font-mono font-bold text-xs flex-shrink-0 {{ $idx === 0 ? 'text-card-yellow' : 'text-text-muted' }}">
                                        #{{ $idx + 1 }}
                                    </span>
                                    <div class="min-w-0">
                                        <span class="font-headline font-bold text-xs sm:text-sm text-text-primary block truncate">{{ $sc->player?->name }}</span>
                                        <span class="font-mono text-[10px] sm:text-[11px] text-text-muted truncate block">{{ $sc->team?->name }} &bull; No. {{ $sc->player?->jersey_number }}</span>
                                    </div>
                                </div>
                                <div class="text-right flex-shrink-0 pl-2">
                                    <span class="font-headline font-black text-base sm:text-lg text-stadium-emerald">{{ $sc->total_goals }}</span>
                    ...
                    <div x-show="statTab === 'assists'" class="p-3 sm:p-4 divide-y divide-court-border/40">
                        @forelse($topAssists as $idx => $as)
                            <div class="py-2.5 flex items-center justify-between gap-2">
                                <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                                    <span class="w-5 sm:w-6 font-mono font-bold text-xs flex-shrink-0 {{ $idx === 0 ? 'text-telemetry-cyan' : 'text-text-muted' }}">
                                        #{{ $idx + 1 }}
                                    </span>
                                    <div class="min-w-0">
                                        <span class="font-headline font-bold text-xs sm:text-sm text-text-primary block truncate">{{ $as->assistPlayer?->name }}</span>
                                        <span class="font-mono text-[10px] sm:text-[11px] text-text-muted truncate block">{{ $as->team?->name }}</span>
                                    </div>
                                </div>
                                <div class="text-right flex-shrink-0 pl-2">
                                    <span class="font-headline font-black text-base sm:text-lg text-telemetry-cyan">{{ $as->total_assists }}</span>
                    ...
                    <div x-show="statTab === 'cards'" class="p-3 sm:p-4 divide-y divide-court-border/40">
                        @forelse($disciplinary as $idx => $card)
                            <div class="py-2.5 flex items-center justify-between gap-2">
                                <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                                    <span class="w-5 sm:w-6 font-mono font-bold text-xs flex-shrink-0 text-text-muted">#{{ $idx + 1 }}</span>
                                    <div class="min-w-0">
                                        <span class="font-headline font-bold text-xs sm:text-sm text-text-primary block truncate">{{ $card->player?->name }}</span>
                                        <span class="font-mono text-[10px] sm:text-[11px] text-text-muted truncate block">{{ $card->team?->name }}</span>
                                    </div>
                                </div>
                                <div class="flex items-center gap-1.5 sm:gap-3 font-mono text-xs flex-shrink-0">
                                    <span class="px-1.5 sm:px-2 py-0.5 rounded bg-card-yellow/20 text-card-yellow border border-card-yellow/40 font-bold text-[10px] sm:text-xs">
                                        🟨 {{ $card->yellow_cards }}
                                    </span>
                                    <span class="px-1.5 sm:px-2 py-0.5 rounded bg-card-red/20 text-card-red border border-card-red/40 font-bold text-[10px] sm:text-xs">
                                        🟥 {{ $card->red_cards }}
                                    </span>
                                </div>
                            </div>
