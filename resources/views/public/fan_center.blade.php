...
@section('title', 'Public Fan Center & Live Match Telemetry - FUTSAL PRO')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8" x-data="fanCenterData({{ $featuredMatch ? $featuredMatch->id : 'null' }})">
    ...
    @if($featuredMatch)
        <div class="relative rounded-xl overflow-hidden bg-court-surface border border-court-border shadow-2xl p-6 sm:p-8">
            <!-- Background Glow Effect -->
            <div class="absolute -top-24 -left-24 w-96 h-96 bg-stadium-emerald/10 rounded-full blur-3xl pointer-events-none"></div>
            <div class="absolute -bottom-24 -right-24 w-96 h-96 bg-telemetry-cyan/10 rounded-full blur-3xl pointer-events-none"></div>

            <div class="relative z-10">
                <!-- Status Top Bar -->
                <div class="flex items-center justify-between pb-6 mb-6 border-b border-court-border/60">
                    <div class="flex items-center gap-3">
                        <span class="px-3 py-1 rounded text-xs font-mono font-bold tracking-wider border {{ $featuredMatch->status_badge['color'] }}" x-text="featured.status_badge.text">
                    ...
                    <div class="flex items-center gap-2 font-mono text-xs">
                        ...
                        <span class="text-telemetry-cyan uppercase">{{ $featuredMatch->stage?->name ?? 'Turnamen' }} {{ $featuredMatch->group ? '('.$featuredMatch->group->name.')' : '' }}</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-11 items-center gap-6 text-center">
                    ...
                    <div class="md:col-span-4 flex flex-col md:flex-row items-center justify-end gap-4">
                        <div class="text-center md:text-right order-2 md:order-1">
                            <h2 class="text-xl sm:text-2xl font-headline font-bold text-text-primary">{{ $featuredMatch->homeTeam->name }}</h2>
                            <p class="text-xs font-mono text-text-muted">KANDANG (HOME)</p>
                        </div>
                        <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-court-surface-elevated border border-court-border flex items-center justify-center font-headline font-black text-2xl text-stadium-emerald shadow-inner order-1 md:order-2">
                        ...
                        <div class="md:col-span-3 flex flex-col items-center justify-center py-4 bg-court-navy/80 rounded-xl border border-court-border/60">
                            <div class="flex items-center gap-4 font-headline text-5xl sm:text-6xl font-black tracking-tight">
                            ...
                            <div class="mt-3 flex items-center gap-2 font-mono text-xs">
                                <span class="px-2.5 py-0.5 rounded font-bold border"
                                      :class="featured.status_badge.color"
                    ...
                    <div class="md:col-span-4 flex flex-col md:flex-row items-center justify-start gap-4">
                        <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-court-surface-elevated border border-court-border flex items-center justify-center font-headline font-black text-2xl text-telemetry-cyan shadow-inner">
                            {{ $featuredMatch->awayTeam->initials }}
                        </div>
                        <div class="text-center md:text-left">
                            <h2 class="text-xl sm:text-2xl font-headline font-bold text-text-primary">{{ $featuredMatch->awayTeam->name }}</h2>
                ...
                <div class="mt-8 pt-6 border-t border-court-border/60 flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('matches.show', $featuredMatch->id) }}" class="px-5 py-2.5 rounded text-xs font-headline font-bold bg-stadium-emerald hover:bg-stadium-emerald/90 text-court-navy uppercase tracking-wider transition-all flex items-center gap-2 shadow-[0_0_15px_rgba(0,255,135,0.25)]">
                            <span class="material-symbols-outlined text-sm">timeline</span>
                            Buka Lini Masa Lengkap & Statistik
                        </a>

                        @auth
                            <a href="{{ route('admin.matches.control', $featuredMatch->id) }}" class="px-4 py-2.5 rounded text-xs font-headline font-bold bg-court-surface-elevated hover:bg-court-border text-telemetry-cyan uppercase tracking-wider border border-court-border transition-all flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-7 space-y-6">
            <div class="bg-court-surface rounded-xl border border-court-border overflow-hidden" x-data="{ activeTab: 'all' }">
                <!-- Tabs Header -->
                <div class="flex items-center justify-between px-6 py-4 border-b border-court-border bg-court-surface-elevated/40">
                    ...
                    <!-- Filter Tabs -->
                    <div class="flex items-center gap-1 bg-court-navy p-1 rounded border border-court-border text-xs font-mono">
                        <button @click="activeTab = 'all'" :class="activeTab === 'all' ? 'bg-court-surface-elevated text-stadium-emerald font-bold' : 'text-text-muted hover:text-text-primary'" class="px-3 py-1 rounded transition-colors">
                            SEMUA ({{ $allMatches->count() }})
                        </button>
                        <button @click="activeTab = 'live'" :class="activeTab === 'live' ? 'bg-court-surface-elevated text-live-pulse font-bold' : 'text-text-muted hover:text-text-primary'" class="px-3 py-1 rounded transition-colors">
                            LIVE ({{ $liveMatches->count() }})
                        </button>
                        <button @click="activeTab = 'upcoming'" :class="activeTab === 'upcoming' ? 'bg-court-surface-elevated text-telemetry-cyan font-bold' : 'text-text-muted hover:text-text-primary'" class="px-3 py-1 rounded transition-colors">
                            TERJADWAL ({{ $upcomingMatches->count() }})
                        </button>
                        <button @click="activeTab = 'finished'" :class="activeTab === 'finished' ? 'bg-court-surface-elevated text-text-primary font-bold' : 'text-text-muted hover:text-text-primary'" class="px-3 py-1 rounded transition-colors">
                ...
                <div class="p-6 space-y-4">
                    @forelse($allMatches as $match)
                        <div x-show="activeTab === 'all' || (activeTab === 'live' && {{ $match->isLive() ? 'true' : 'false' }}) || (activeTab === 'upcoming' && '{{ $match->status }}' === 'scheduled') || (activeTab === 'finished' && '{{ $match->status }}' === 'finished')"
                            ...
                            <div class="flex items-center justify-between text-[11px] font-mono text-text-muted pb-3 mb-3 border-b border-court-border/40">
                                <span>{{ $match->match_date->format('d M Y - H:i') }} WIB &bull; {{ $match->venue }}</span>
                                <span class="px-2 py-0.5 rounded text-[10px] font-bold border {{ $match->status_badge['color'] }}">
                            ...
                            <div class="grid grid-cols-7 items-center gap-2">
                                ...
                                <div class="col-span-3 flex items-center gap-3">
                                    <div class="w-8 h-8 rounded bg-court-navy border border-court-border flex items-center justify-center font-headline font-bold text-xs text-stadium-emerald flex-shrink-0">
                                        {{ $match->homeTeam->initials }}
                                    </div>
                                    <span class="font-headline font-bold text-sm text-text-primary truncate">{{ $match->homeTeam->name }}</span>
                                </div>
                                ...
                                <div class="col-span-3 flex items-center justify-end gap-3 text-right">
                                    <span class="font-headline font-bold text-sm text-text-primary truncate">{{ $match->awayTeam->name }}</span>
                                    <div class="w-8 h-8 rounded bg-court-navy border border-court-border flex items-center justify-center font-headline font-bold text-xs text-telemetry-cyan flex-shrink-0">
                            ...
                            <!-- Actions bar -->
                            <div class="mt-3 pt-3 border-t border-court-border/40 flex items-center justify-between text-xs font-mono">
                                <span class="text-text-muted text-[11px]">
                                    {{ $match->stage?->name }} {{ $match->group ? '&bull; '.$match->group->name : '' }}
                                </span>
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('matches.show', $match->id) }}" class="text-stadium-emerald hover:underline text-xs flex items-center gap-1 font-headline font-medium">
                                ...
                                @forelse($bStage['matches'] as $bm)
                                    <div class="p-2.5 rounded bg-court-navy/70 border border-court-border/60 text-xs">
                                        <div class="flex justify-between items-center py-1">
                                            <span class="font-headline font-bold text-text-primary">{{ $bm->homeTeam->name }}</span>
                                            <span class="font-mono font-black text-stadium-emerald">{{ $bm->home_score }}</span>
                                        </div>
                                        <div class="flex justify-between items-center py-1 border-t border-court-border/30">
                                            <span class="font-headline font-bold text-text-primary">{{ $bm->awayTeam->name }}</span>
                                            <span class="font-mono font-black text-telemetry-cyan">{{ $bm->away_score }}</span>
                                        </div>
                                    </div>
                                @empty
        ...
        <div class="lg:col-span-5 space-y-6">
            ...
            <div class="bg-court-surface rounded-xl border border-court-border overflow-hidden" 
                ...
                <!-- Standings Header -->
                <div class="px-6 py-4 border-b border-court-border bg-court-surface-elevated/40 flex items-center justify-between">
                    <div>
                        <h3 class="font-headline font-bold uppercase tracking-wider text-sm flex items-center gap-2">
                ...
                @foreach($standingsByGroup as $gName => $gData)
                    <div x-show="activeGroup === '{{ $gName }}'" class="overflow-x-auto">
                        @if(count($standingsByGroup) > 1)
                            <div class="px-6 py-2 bg-court-surface-elevated/20 border-b border-court-border text-xs font-headline font-bold text-telemetry-cyan uppercase">
                                {{ $gName }}
                            </div>
                        @endif
                                ...
                                @forelse($gData['rows'] as $row)
                                    <tr class="hover:bg-court-surface-elevated/40 transition-colors {{ $row['position'] <= 2 ? 'border-l-2 border-l-stadium-emerald' : '' }}">
                                        ...
                                        <td class="py-3 px-3 font-sans font-medium text-text-primary">
                                            <div class="flex items-center gap-2">
                                                <div class="w-6 h-6 rounded bg-court-navy border border-court-border flex items-center justify-center font-headline font-bold text-[10px] text-stadium-emerald">
                                                    {{ $row['team']->initials }}
                                                </div>
                                                <span class="truncate max-w-[130px] sm:max-w-none">{{ $row['team']->name }}</span>
                                            </div>
                                        </td>
                ...
                @endforeach
                <div class="px-4 py-2.5 bg-court-navy/50 border-t border-court-border text-[10px] font-mono text-text-muted flex items-center justify-between">
                    <span class="flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded bg-stadium-emerald"></span>
                        Posisi 1 - 2 Lolos ke Babak Knockout
                    </span>
            ...
            <div class="bg-court-surface rounded-xl border border-court-border overflow-hidden" x-data="{ statTab: 'scorers' }">
                <div class="px-6 py-4 border-b border-court-border bg-court-surface-elevated/40 flex items-center justify-between">
                    <h3 class="font-headline font-bold uppercase tracking-wider text-sm flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-1 bg-court-navy p-1 rounded border border-court-border text-xs font-mono">
                        <button @click="statTab = 'scorers'" :class="statTab === 'scorers' ? 'bg-stadium-emerald text-court-navy font-bold' : 'text-text-muted hover:text-text-primary'" class="px-2.5 py-1 rounded transition-colors">
                            GOL
                        </button>
                        <button @click="statTab = 'assists'" :class="statTab === 'assists' ? 'bg-telemetry-cyan text-court-navy font-bold' : 'text-text-muted hover:text-text-primary'" class="px-2.5 py-1 rounded transition-colors">
                            ASSIST
                        </button>
                        <button @click="statTab = 'cards'" :class="statTab === 'cards' ? 'bg-card-yellow text-court-navy font-bold' : 'text-text-muted hover:text-text-primary'" class="px-2.5 py-1 rounded transition-colors">
                            KARTU
                        </button>
                    </div>
                </div>
                ...
                <div x-show="statTab === 'scorers'" class="p-4 divide-y divide-court-border/40">
                    @forelse($topScorers as $idx => $sc)
                        <div class="py-2.5 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <span class="w-6 font-mono font-bold text-xs {{ $idx === 0 ? 'text-card-yellow' : 'text-text-muted' }}">
                                    #{{ $idx + 1 }}
                                </span>
                                <div>
                                    <span class="font-headline font-bold text-sm text-text-primary block">{{ $sc->player?->name }}</span>
                                    <span class="font-mono text-[11px] text-text-muted">{{ $sc->team?->name }} &bull; No. {{ $sc->player?->jersey_number }}</span>
                                </div>
                            </div>
                            <div class="text-right">
                                <span class="font-headline font-black text-lg text-stadium-emerald">{{ $sc->total_goals }}</span>
                ...
                <div x-show="statTab === 'assists'" class="p-4 divide-y divide-court-border/40">
                    @forelse($topAssists as $idx => $as)
                        <div class="py-2.5 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <span class="w-6 font-mono font-bold text-xs {{ $idx === 0 ? 'text-telemetry-cyan' : 'text-text-muted' }}">
                                    #{{ $idx + 1 }}
                                </span>
                                <div>
                                    <span class="font-headline font-bold text-sm text-text-primary block">{{ $as->assistPlayer?->name }}</span>
                                    <span class="font-mono text-[11px] text-text-muted">{{ $as->team?->name }}</span>
                                </div>
                            </div>
                            <div class="text-right">
                                <span class="font-headline font-black text-lg text-telemetry-cyan">{{ $as->total_assists }}</span>
                ...
                <div x-show="statTab === 'cards'" class="p-4 divide-y divide-court-border/40">
                    @forelse($disciplinary as $idx => $card)
                        <div class="py-2.5 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <span class="w-6 font-mono font-bold text-xs text-text-muted">#{{ $idx + 1 }}</span>
                                <div>
                                    <span class="font-headline font-bold text-sm text-text-primary block">{{ $card->player?->name }}</span>
                                    <span class="font-mono text-[11px] text-text-muted">{{ $card->team?->name }}</span>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 font-mono text-xs">
                                <span class="px-2 py-0.5 rounded bg-card-yellow/20 text-card-yellow border border-card-yellow/40 font-bold">
                                    🟨 {{ $card->yellow_cards }}
                                </span>
                                <span class="px-2 py-0.5 rounded bg-card-red/20 text-card-red border border-card-red/40 font-bold">
                                    🟥 {{ $card->red_cards }}
                                </span>
                            </div>
                        </div>
